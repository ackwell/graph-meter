...
function Graph() {
	...
	this.axis = config.showAxis? new Axis(this.graph) : null;

	// Set up a resize handler and do an initial call
	$(window).resize(function() {
		this.graph.configure({
			width: window.innerWidth - (config.showAxis? config.axisWidth : 0),
...
Graph.prototype.render = function() {
	this.graph.render();
	if (this.axis) { this.axis.render(); }
}

Graph.prototype.reset = function() {
	this.series.length = 0;
	this.map = {};
	if (this.axis) { this.axis.reset(); }
}
